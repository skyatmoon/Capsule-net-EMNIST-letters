...
def test(model, test_loader, args):
    ...
    for x, y in test_loader:
        y = torch.zeros(y.size(0), 27).scatter_(1, y.view(-1, 1), 1.)
...
def train(model, train_loader, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (x, y) in enumerate(train_loader):  # batch training
            ...
            y = torch.zeros(y.size(0), 27).scatter_(1, y.view(-1, 1), 1.)  # change to one-hot coding
            #y = one_hot[0]
            #print(y)
...
if __name__ == "__main__":
    ...
    # define model
    model = CapsuleNet(input_size=[1, 28, 28], classes=27, routings=3)
    model.cuda()
    print(model)
